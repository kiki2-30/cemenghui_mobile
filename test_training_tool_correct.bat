@echo off
echo 测试TrainingDetail和ToolDetail的结构化字段适配（修正版）...

echo.
echo 1. 登录获取token
curl -X POST "http://localhost:8088/user/login" -H "Content-Type: application/json" -d "{\"username\":\"superadmin\",\"password\":\"superadmin123\"}"
